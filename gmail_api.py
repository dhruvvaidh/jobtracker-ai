...
# For every type of email, the payload is structured differently
# For nested emails, the emails are nested in the body
def _extract_body (payload):
            ...
            if part['mimeType'] == 'multipart/alternative':
                for subpart in part['parts']:
                    if subpart['mimeType'] == 'text/plain' and 'data' in subpart ['body']:
                        # Emails are recieved in base64 encoded format so we need to decode them
    ...
    return body
def get_email_messages(service, user_id='me', label_ids=None, folder_name='INBOX', max_results=5, query="newer_than:1d"):
    messages = []
    ...
    while True:
        result = service.users().messages().list(
            userId=user_id,
            labelIds=label_ids,
            q=query,  # Apply the date range query.
